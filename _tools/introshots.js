...
const LINKS = [['Fit-Out & Finishes','intro-p4'],['Handover & Aftercare','intro-p5'],['Project Tour','intro-p6']];
(async () => {
  ...
  for (const [name, tag] of LINKS) {
    await page.click('#menu-icon'); await new Promise(r => setTimeout(r, 1000));
    const ok = await page.evaluate((n) => { const li=[...document.querySelectorAll('#menu-links *')].find(e=>e.textContent.trim()===n||e.textContent.includes(n)); if(li){li.click();return true;} return false; }, name);
    await new Promise(r => setTimeout(r, 4000));
    await page.screenshot({ path: `${OUT}/${tag}.png` });
    console.log(tag, ok ? 'shot' : 'LINK NOT FOUND');
  }
  await browser.close();
})();
